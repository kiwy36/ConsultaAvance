...
const CartWidget = () => {
    ...
    const handleFinishPurchase = () => {
        ...
        if (!name || !/^[a-zA-Z\s]+$/.test(name)) {
            setErrorMessage("El Nombre y apellido no esta completados correctamente");
            return;
        }
        if (!cardNumber || !/^\d{16}$/.test(cardNumber)) {
            setErrorMessage("El Número de tarjeta no esta completado correctamente (debe contener 16 dígitos)");
            return;
        }
        if (!securityCode || !/^\d{3}$/.test(securityCode)) {
            setErrorMessage("El Código de seguridad no esta completado correctamente (debe contener 3 dígitos)");
            return;
        }

        setErrorMessage(""); // Limpiar el mensaje de error si todo está correcto

        alert(`¡Compra terminada! Total: $${total}`);
        clearCart(); // Eliminar los elementos del carrito
    };
